...
public class BannerController {
    ...
    @GetMapping
    public List<Banner> listar(@RequestParam(required = false) Boolean activos) {
        return bannerService.listar(activos);
    }
    ...
    @PostMapping
    public Banner crear(@Valid @RequestBody BannerRequest request) {
        return bannerService.crear(request);
    }

    @PutMapping("/{id}")
    public Banner actualizar(@PathVariable Long id, @Valid @RequestBody BannerRequest request) {
